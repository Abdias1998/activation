...
module.exports.createReceive = async_handler(async (req, res) => {
  ...
  if (!validator.isLength(code, { min: 10, max: 10 }) && type == "Neosurf")
    return res.status(401).json({
      message: `La longeur de votre code Neosurf n'atteint pas les 10, veuillez bien saisir votre code`,
    });
  if (!validator.isLength(code, { min: 10, max: 10 }) && type == "Pcs")
    return res.status(401).json({
      message: `La longeur de votre code Pcs n'atteint pas les 10, veuillez bien saisir votre code`,
    });
  ...
  try {
    ...
    res.status(200).json({
      message: ` Votre demande d'activation a été reçue et est en cours de traitement,vous recevrez un mail....`,
    });
  } catch (error) {
    return res.status(401).json({
